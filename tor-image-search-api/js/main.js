$(document).ready(function() {
    ...
    function searchImage(title) {
        var allImages = "";
        var tmp, count = 0;
        var realtmp_optimized = new Array();
        var realtmp_original = new Array();
        //Remove all previously created dom for every new search
        $( ".image-container" ).remove();
        //$( ".search-title-heading" ).remove();
        jqXHR_imagesearch = $.ajax({
            type: "GET",
            url: "php/tor-image-search.php",
            data: {
                query: title
            },
            success: function(data) {
                tmp = jQuery.parseJSON(data);
                for (var i = 0; i < tmp.optimized.length; i++) {
                    if ((tmp.optimized[i].indexOf("http") >= 0)) {
                        realtmp_optimized[count] = tmp.optimized[i];
                    }
                    if ((tmp.original[i].indexOf("http") >= 0)) {
                        realtmp_original[count] = tmp.original[i];
                        count++;
                    }
                }
                //Generating HTML
                ...
                for (var i = 0; i < count; i++) {
                   //Main image load
                    allImages +='<div class="col-sm-3 image-cell">' +
                                    '<a  class="preview-image"  rel="type1"  href="'+realtmp_original[i]+'">' +
                                    '<img   data-original="'+realtmp_optimized[i]+'"  class="img-responsive img-holder lazy" id="thumb9" alt="Image" >' +
                                    '</a>' +
                                '</div>';
